import Observe from '../spotify/observe'
import * as cookies from '../spotify/cookie'
import { get } from '../spotify/network'
import interpolate from '../spotify/interpolate'
import { scaleLog } from 'd3-scale'
import { min } from 'd3-array'
import ease from '../spotify/easing'
...
export default class Sync {
   ...
   */
  async getNewToken () {
    const { data } = await get(`${process.env.PROJECT_ROOT}/api/spotify/refresh?token=${this.state.api.tokens.refreshToken}`)
...
export async function auth () {
  const { data } = await get(`/api/spotify/auth`)
  window.location.href = `/api/spotify/login?auth_id=${data.auth_id}`
}
